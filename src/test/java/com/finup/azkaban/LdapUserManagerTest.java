...
private LdapUserManager userManager;
    ...
    private Props getProps() {
        ...
        props.put(LdapUserManager.LDAP_HOST, "freeipa1.cdh.com");
        props.put(LdapUserManager.LDAP_PORT, "389");
        props.put(LdapUserManager.LDAP_USE_SSL, "false");
        props.put(LdapUserManager.LDAP_USER_BASE, "dc=cdh,dc=com");
        props.put(LdapUserManager.LDAP_USERID_PROPERTY, "uid");
        props.put(LdapUserManager.LDAP_EMAIL_PROPERTY, "mail");
        props.put(LdapUserManager.LDAP_BIND_ACCOUNT, "uid=admin,cn=users,cn=accounts,dc=cdh,dc=com");
        props.put(LdapUserManager.LDAP_BIND_PASSWORD, "1q2w3e!Q@W#E");
        props.put(LdapUserManager.LDAP_ALLOWED_GROUPS, "");
        props.put(LdapUserManager.LDAP_GROUP_SEARCH_BASE, "cn=groups,cn=accounts");
        return props;
    }

    @Test
    public void testGetUser() throws Exception {
        User user = userManager.getUser("test025337", "1234qwer");

        assertEquals("test025337", user.getUserId());
